...
export class MyAccountPage {
  ...
  registerButton = this.page.locator(
    '//*[@class="woocommerce-Button button"][@name="register"]',
  );
  userNameText = this.page.locator(
    '//*[@class="woocommerce-MyAccount-content"]/p/strong[1]',
  );
  confirmationStrongPasswordText = this.page.locator(
    '//*[@class="woocommerce-password-strength strong"]',
  );
  ...
  billingCountryContainer = this.page.locator(
    '#select2-billing_country-container',
  );
  listbillingCountry = this.page.locator(
    '//*[@id="select2-billing_country-results"]',
  );
  ...
  confirmationAddressChangeText = this.page.locator(
    '//*[@class="woocommerce-message"]',
  );
  errorMessage = this.page.locator('//*[@class="woocommerce-error"]');
  menuAccountOrders = this.page.getByRole('link', {
    name: 'Orders',
    exact: true,
  });
  firstCellonOrderPage = this.page.locator(
    '#woocommerce-orders-table__row woocommerce-orders-table__row--status-processing order',
  );
  tableOnTheOrderPage = this.page.getByText(
    'Order Date Status Total Actions #7383 February 29, 2024 Processing 212.00 zł',
  );
